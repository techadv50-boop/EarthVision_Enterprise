...
def test_dump_databases_uses_docker_exec_for_container_schema(tmp_path: Path, monkeypatch):
    ...
    assert captured[0][:4] == ["docker", "exec", "sea50-cyfdw1-db-1", "sh"]
    ...
    assert "sea50_db" in captured[0]


def test_dump_databases_container_script_retries_with_env_password(tmp_path: Path, monkeypatch):
    import docker_db

    argv = docker_db.docker_dump_argv("sea50-cyfdw1-db-1", "sea50_db")
    assert argv[:5] == ["docker", "exec", "sea50-cyfdw1-db-1", "sh", "-c"]
    assert "MYSQL_PWD=" in argv[5]
    assert "-uroot" in argv[5]
    assert "sea50_db" == argv[-1]
