"""Task-specific EO detectors for Light Explorer (CV + classical ML — not random overlays).

Each product uses a specialized algorithm family:
  • Buildings / urban: Morphological Building Index (MBI)-style NDBI + brightness morphology
...
TASK_META: dict[str, dict[str, Any]] = {
    "building_detection": {
        ...
        "domain": "ai",
        "detector": "buildings",
        "algorithm": "Morphological Building Index (NDBI×brightness + open/close + contours)",
        "needs_scene": True,
    },
    "road_extraction": {
        "label": "Road",
    ...
    "urban_expansion_detection": {
        ...
        "domain": "ai",
        "detector": "buildings",
        "algorithm": "NDBI built-up clusters (Zha urban index morphology)",
        "needs_scene": True,
    },
...
class DetectionService:
    ...
    def _det_buildings(self, task, label, bands, bounds, conf_min):
        ...
        if ndbi is None:
            raise ValidationError("Building detection needs NIR+SWIR bands")
        # Morphological Building Index style: high NDBI, low NDVI, bright
        score = self._norm01(ndbi)
        if ndvi is not None:
            score = score * (1.0 - 0.55 * self._norm01(ndvi))
        score = score * (0.5 + 0.5 * self._norm01(inten))
        u8 = self._to_u8(score)
        # Top-hat morphological enhancement (MBI-like)
        ...
        return self._contour_polygons(
            mask, score, bounds, label, task, conf_min, min_area=25, max_features=80
        )
