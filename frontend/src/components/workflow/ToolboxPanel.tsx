        ...
        <div className="mb-2">
          ...
          {['ai', 'maritime', 'aviation'].includes(activeBox.id) && (
            <p className="mt-1 rounded border border-[var(--line)] bg-[var(--bg)] px-2 py-1 text-[10px] text-[var(--muted)]">
              {activeBox.id === 'ai'
                ? 'AI tools stay inactive except Ship Detection. Activate with Landsat/Sentinel-2 eye ON, then draw a Rect/Poly AOI — GEE Optimized NIR≥0.10 (AOI-first) marks ships with a thick red outline (no shapefile download).'
